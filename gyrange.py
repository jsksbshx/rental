    ...
    if user_id in admin_id:
        ...
        if len(command) > 2:
            ...
            try:
                ...
                response = "Thik se daalo. Please provide a positive integer followed by 'hour(s)', 'day(s)', 'week(s)', or 'month(s)'."
        ...
        response = "Mood ni hai abhi pelhe purchase kar isse:- @saiyam_96."
...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karlo:- @saiyam_96."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"CHUDAI start : {target}:{port} for {time}\nSEC Jabtak YE Attack run krrha hai.Koi aur na lagana"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "Abhi Attack Chalu hai. Thoda sabar kar pehle jab wo khatam ho."
            bot.reply_to(message, response)
            return
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "YOU ARE NOT A PREMIUM USER ❌ ."
...
def show_help(message):
    help_text ='''
💥 /chodo : 😫FOR SERVER FREEZE 🥵. 
💥 /rules : 📒GWAR RULES PADHLE KAM AYEGA📒 !!.
💥 /mylogs : 👁️SAB ATTACKS DEKLO👁️.
💥 /plan : 💵SABKE BSS KA BAT HAI💵.
💥 /myinfo : 📃APNE PLAN KI VEDHTA DEKHLE📃.
...
def welcome_start(message):
    ...
    response = f'''🔥saiyam ke DDOS pe aapka swagat hai, {user_name}! . Kharidne ke liye Kira se sampark karein.
